...
The workshop was enhanced in 2021 with aspects from the https://github.com/msgoat/cxp-dox-home/[Cloud Expert Program
of msg].

== Fundamental Concepts

* Docker-Daemon runs in linux kernel and is the base for Docker images to run on; can be controlled using CLI
...
* "docker run" runs interactively. To run as daemon in background:
----
docker run -D
----
...
** run creates new container with _docker create_ from an image (which is downloaded if not existing in local
repository) and executes container
